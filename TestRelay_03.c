...
void app_main(void)
{
  ...
  relay_config.pin_bit_mask = (1ULL << RELAY1_PIN) | (1ULL << RELAY2_PIN) | (1ULL << RELAY3_PIN); // Seleciona os pinos GPIO
  relay_config.pull_up_en = GPIO_PULLUP_DISABLE;                                                  // Desabilita pull up
  relay_config.pull_down_en = GPIO_PULLDOWN_ENABLE;                                               // Habilita pull down
